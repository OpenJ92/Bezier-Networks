...
if __name__ == '__main__':
    ...
    # look to introduce the reparamateration trick to our model. Maybe we'll be able to
    # with such a term introduced.
    ...
    X1 = NavBotData('seriesID.npy', 'train_X/tensor_order_1', transforms.Compose([NGRAMDomain(shape_init[0]), MINMAX()]))
    XT = NavBotData('seriesID.npy', 'train_X/tensor_order_T', transforms.Compose([NGRAMDomain(shape_init[0]), MINMAX()]))
    ...
    data = torch.from_numpy(X1[:5]).float()
