...
from convLinearInterpolation import Conv2dInterpolation
...
class conv2dbezierNetwork():
    ...
    def __init__(self, shape_in, shape_out, control_points, bezier_samples, layers):
        ...
        self.callable_network = self.construct_Sequential_Networks()

    def __call__(self, A):
        NET = self.callable_network
        for net in NET:
            A = net(A)
        return A
...
if __name__ == "__main__":
    ...
    # control_points = controlPointsVertebralWalk(shape_in, shape_out, np.array([1,1,1]))(20)
